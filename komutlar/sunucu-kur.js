...
message.guild.roles.create({ data: { name: 'kairos Prensesleri 🍓' }, reason: 'kairos' }).then(role => {
    role.setPermissions([]);
    role.setColor('#ff1b5f');
    });

message.guild.roles.create({ data: { name: '</>' }, reason: 'kairos' }).then(role => {
    role.setPermissions([]);
...
message.guild.roles.create({ data: { name: 'See Banner 🎈' }, reason: 'kairos' }).then(role => {
    role.setPermissions([]);
    role.setColor('#000000');
    });

message.guild.roles.create({ data: { name: '🔞NSFW' }, reason: 'kairos' }).then(role => {
    role.setPermissions([]);
    role.setColor('#000000');
    });

message.guild.roles.create({ data: { name: 'Muted' }, reason: 'kairos' }).then(role => {
    role.setPermissions([]);
    role.setColor('#000000');
    });
